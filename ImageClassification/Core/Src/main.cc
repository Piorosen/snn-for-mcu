...
//  osKernelStart();
  ...
  // 정답 데이터는 [0 부터 9] 까지 순서대로 정렬한 상태임.
  char file_name[30];
  ...
  while (true) {
	  for (int loop_image = 0; loop_image < 10; loop_image++) {
		  for (int i = 0; i < 10; i++){
			  calc[i] = 0;
		  }
		  ...
		  for (int t = 0; t < 30; ++t) {
			  ...
			  snprintf(lcd_output_string, sizeof(lcd_output_string),
					   "  Pred : [%d, %s], Ans : [%d, %s]       ", pred, labels[pred], loop_image, labels[loop_image]);
		...
		//      BSP_LCD_DisplayStringAt(0, LINE(5), (uint8_t*)lcd_output_string, LEFT_MODE);
		  ...
		  sprintf(lcd_output_string, "  ----- Previous Result -----");
		  ...
		  BSP_LCD_DisplayStringAt(0, LINE(13), (uint8_t*)lcd_output_string, LEFT_MODE);

		  sprintf(lcd_output_string, "  Pred : [%d, %s], Ans : [%d, %s]       ", pred, labels[pred], loop_image, labels[loop_image]);
		  BSP_LCD_DisplayStringAt(0, LINE(14), (uint8_t*)lcd_output_string, LEFT_MODE);
